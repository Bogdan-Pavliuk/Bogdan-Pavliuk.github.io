...
function renderData(data) {
  ...
  dataBlock.innerHTML = data
    .map(
      ({ date, tips }) =>
        `<div class='block'><div class='date'>${date}</div><div class='tips'>${tips}</div></div>`
    )
    .join("");
}
...
function addEventDeleteOneBlock(render) {
  ...
  for (let i = 0; i < dataBlockItem.length; i++) {
    dataBlockItem[i].addEventListener("click", function () {
      if (
        confirm(
          `Хочеш видалити чай: ${this.children[1].innerHTML}? (${this.children[0].innerHTML})`
        )
      ) {
        const data = getItem("data");
...
function renderSumTips(data) {
  ...
  return `<div id='result_sum'>Чай - ${
    showMoney ? data.reduce((ac, { tips }) => ac + tips, 0) : "?"
  } грн</div>`;
}
